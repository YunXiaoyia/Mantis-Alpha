...
"""
SmolVLA policy implementation for Mantis-Alpha.

[Paper](https://huggingface.co/papers/2506.01844)

The model code follows the Hugging Face SmolVLA design (Apache 2.0) and is
vendored into this repository so that Mantis-Alpha runs without importing
LeRobot.

Train on a LeRobot-format dataset (see scripts/train.py):

```bash
python scripts/train.py \
    --dataset_root /path/to/libero \
    --vlm_model_name /path/to/smolvlm2 \
    --batch_size 8 \
    --steps 100
```
...
```python
from mantis_alpha import SmolVLAPolicy

policy = SmolVLAPolicy.from_pretrained("outputs/checkpoints/010000")
```
...
from .policy_base import PreTrainedPolicy
